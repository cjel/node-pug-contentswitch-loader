...
module.exports = function parentScopeLoader(source) {
  ...
  mode = false
  modes = ['t3template', 'styleguide', 'template']
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith(hitindicator)) {
      ...
      if (line.trim().replace(hitindicator, '').startsWith(indicator)) {
        ...
        if (mode == 'styleguide') {
          result.push(' '.repeat(depth - 2) + '| ' + text)
          //result.push(' '.repeat(depth) + '| ' + text)
          //result.push(' '.repeat(depth - 2) + '| ' + mode)
        }
      }
      ...
      if (line.trim().replace(hitindicator, '').startsWith(indicator)) {
        preventPush = true
        var linedata = line.trim().replace(hitindicator + indicator, '').trim()
        result.push(' '.repeat(depth) + '<!--|%|' + linedata + '|%|-->')
      }
    }
